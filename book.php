    ...
    const fetchData = async (hotelID) => {
        ...
        try {
            ...
            if (data) {
                ...
                div.innerHTML = `
                    <img src="<?php echo base_url('/assests/')?>${data[0][0].poster}" alt="">
                    <div class="rightImages">
                     <img src="<?php echo base_url('/assests/');?>${data[0][0].room_andHotelImages.split(',')[0]}"alt="">
                       <img src="<?php echo base_url('/assests/');?>${data[0][0].room_andHotelImages.split(',')[1]}"alt="">
                    </div>
...
const halfStar = data[0][0].rate % 1 >= 0.5 ? 1 : 0;
...
bookingDetails.innerHTML = `
  ...
  <div class="hotelBookCheckIN-out">
    <div class="checkIN">
      ...
      <h6>${formatDateParts(date1).weekday} <span>${formatDateParts(date1).day}
       ${formatDateParts(date1).month}</span> ${formatDateParts(date1).year}</h6>
      <p>12 PM</p>
    </div>
    ...
    <div class="checkIN">
      ...
      <h6>${formatDateParts(date2).weekday} <span>${formatDateParts(date2).day}
       ${formatDateParts(date2).month}</span> ${formatDateParts(date2).year}</h6>
      <p>12 PM</p>
    </div>

    <div class="checkIN checkIN2">
    <span><b>${nights}</b> Nights</span>
    <span>${peopleValue}</span> People
...
let checkLogin = <?php echo json_encode($this->session->userdata('userdata')
['id'] ?? null);  ?>
...
let roomBook = async ( 
    hotelID, hotelName, startDate, endDate, userID, price,
     peopleValue, discount ,bookingName,  bookingEmail,
     bookingPhone, nights
    ) => {

    let data = {
         hotelID, hotelName, startDate, endDate,
          userID, price, peopleValue, discount ,bookingName,
          bookingEmail, bookingPhone, nights
    }
